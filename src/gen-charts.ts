...
export function createExcelWorksheet(
    ...
    return new Promise((resolve, reject) => {
        ...
        // sharedStrings.xml
        {
            ...
            if (chartObject.opts.type === CHART_TYPES.BUBBLE) {
                ...
                data.forEach(objData => {
                    strSharedStrings +=
                        '<si><t>' +
                        encodeXmlEntities(
                            (objData.name || ' ').replace('X-Axis', 'X-Values')
                        ) +
                        '</t></si>'
                })
            }
            ...
            if (
                chartObject.opts.type !== CHART_TYPES.BUBBLE &&
        ...
        zipExcel
            ...
            .then(content => {
                ...
                zip.file(
                    'ppt/charts/' + chartObject.fileName,
...
export function makeXmlCharts(rel: ISlideRelChart): string {
    ...
    if (Array.isArray(rel.opts.type)) {
        rel.opts.type.forEach(type => {
            ...
            let valAxisId = options['secondaryValAxis']
                ? AXIS_ID_VALUE_SECONDARY
                : AXIS_ID_VALUE_PRIMARY
            let catAxisId = options['secondaryCatAxis']
                ? AXIS_ID_CATEGORY_SECONDARY
                : AXIS_ID_CATEGORY_PRIMARY
    ...
    if (
        ...
        if (rel.opts.catAxes) {
            ...
            strXml += makeCatAxis(
                getMix(rel.opts, rel.opts.catAxes[0]) as IChartOpts,
                AXIS_ID_CATEGORY_PRIMARY,
                AXIS_ID_VALUE_PRIMARY
            )
            if (rel.opts.catAxes[1]) {
                strXml += makeCatAxis(
                    getMix(rel.opts, rel.opts.catAxes[1]) as IChartOpts,
                    AXIS_ID_CATEGORY_SECONDARY,
                    AXIS_ID_VALUE_PRIMARY
                )
            }
        ...
        if (rel.opts.valAxes) {
            strXml += makeValAxis(
                getMix(rel.opts, rel.opts.valAxes[0]) as IChartOpts,
                AXIS_ID_VALUE_PRIMARY
            )
            if (rel.opts.valAxes[1]) {
                strXml += makeValAxis(
                    getMix(rel.opts, rel.opts.valAxes[1]) as IChartOpts,
                    AXIS_ID_VALUE_SECONDARY
                )
            }
        } else {
        ...
        case CHART_TYPES.RADAR:
			...
			*/
            ...
            data.forEach(obj => {
                ...
                // Fill and Border
                let strSerColor = opts.chartColors
                ...
                if (chartType !== CHART_TYPES.RADAR) {
                    ...
                    strXml +=
                        '          <a:solidFill>' +
                        createColorElement(
                            opts.dataLabelColor || DEF_FONT_COLOR
                        ) +
                        '</a:solidFill>'
                    strXml +=
                        '          <a:latin typeface="' +
                ...
                if (
                    ...
                    let symbolLineColor =
                        opts.lineDataSymbolLineColor || strSerColor
                    strXml +=
                        '    <a:ln w="' +
        ...
        case CHART_TYPES.BUBBLE:
			...
			*/
            ...
            data.filter((_obj, idx) => {
                ...
                strXml += '    <c:strRef>'
                strXml +=
                    '      <c:f>Sheet1!$' + LETTERS[idxColLtr] + '$1</c:f>'
                strXml +=
                    '      <c:strCache><c:ptCount val="1"/><c:pt idx="0"><c:v>' +
                    obj.name +
                    '</c:v></c:pt></c:strCache>'
                ...
                strXml +=
                    ...
                    createColorElement(
                        opts.chartColors[
                            idx + 1 > opts.chartColors.length
                                ? Math.floor(
                                      Math.random() * opts.chartColors.length
                                  )
                                : idx
                        ]
                    ) +
    ...
    if (opts.catLabelFormatCode) {
        ;[
            ...
            if (
                ...
                console.warn(
                    '`' + opt + "` must be one of: 'days','months','years' !"
                )
                opts[opt] = null
            }
        })
