			...
			data.forEach(obj => {
				...
				if ((chartType === CHART_TYPE.BAR || chartType === CHART_TYPE.BAR3D) && (data.length === 1 || opts.valueBarColors)) {
					...
					obj.values.forEach((value, index) => {
						...
						if (data.length > 1 && opts.chartColors && opts.chartColors.length > obj.values.length) {
						...
						if (obj.valueLabels && opts.dataLabelFormatBar == 'custom') {
							...
							strXml +='<c:dLbls>';
								...
								if (opts.dataLabelFormatBar == 'custom') {
									strXml +='  <c:dLbl>';
									strXml += '    <c:numFmt formatCode="'+ opts.dataLabelFormatCode +'" sourceLinked="0"/>';
									strXml +='    <c:idx val="'+ (index % obj.valueLabels.length) +'"/>';
									strXml +='    <c:tx>';
									strXml +='      <c:rich>';
									strXml +='			<a:bodyPr>';
									strXml +='				<a:spAutoFit/>';
									...
									strXml +='            		<a:t>'+ ' ' + encodeXmlEntities(obj.valueLabels[colorIndex % obj.valueLabels.length]) +'</a:t>';
									strXml +='          	</a:r>';
