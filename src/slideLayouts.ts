...
export class Master {
    ...
    configureBackground(bkg) {
        if (typeof bkg === 'object' && (bkg.src || bkg.path || bkg.data)) {
            ...
            this.bkgdImgRid = this.relations.registerImage(
                { data: bkg.data, path: bkg.src },
                strImgExtn
            )
        ...
        if (
            ...
            slideDef.objects.forEach((object, idx: number) => {
                ...
                if (MASTER_OBJECTS[key] && key === 'chart') {
                    ...
                    this.data.push(
                        new ImageElement(object[key], this.relations)
                    )
                } else if (MASTER_OBJECTS[key] && key === 'line') {
                    this.data.push(
                        new ShapeElement(BASE_SHAPES.LINE, object[key])
                    )
                } else if (MASTER_OBJECTS[key] && key === 'rect') {
                    this.data.push(
                        new ShapeElement(BASE_SHAPES.RECTANGLE, object[key])
                    )
                    ...
                    console.log('slide number', object[key].options)
                    this.data.push(
                    ...
                    let placeholder
                    if (options.type === 'pic') {
                        placeholder = new PlaceholderImageElement(
                            options,
                            100 + idx
                        )
                    } else {
                        placeholder = new PlaceholderTextElement(
                            text,
                            options,
                            100 + idx,
                            this.relations
                    ...
                    if (this.placeholders.has(placeholder.name)) {
                        console.warn(
                            `Duplicate placeholders with name "${placeholder.name}"`
                        )
                        return
                    }
