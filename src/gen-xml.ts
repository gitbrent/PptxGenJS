...
function slideObjectToXml(slide: ISlide | ISlideLayout): string {
    ...
    slide.data.forEach(
        (slideItemObj: ISlideObject | TextElement, idx: number) => {
            ...
            if (
                (slide as ISlide).slideLayout !== undefined &&
                (slide as ISlide).slideLayout.data !== undefined &&
                slideItemObj.options &&
                slideItemObj.options.placeholder
            ) {
                placeholderObj = slide['slideLayout']['data'].filter(
                    (object: ISlideObject) => {
                        return (
                            object.options.placeholder ===
                            slideItemObj.options.placeholder
                        )
                    }
                )[0]
            ...
            switch (slideItemObj.type) {
                case SLIDE_OBJECT_TYPES.table:
                    ...
                    arrTabRows[0].forEach(cell => {
                        ...
                        intColCnt +=
                            cellOpts && cellOpts.colspan
                                ? Number(cellOpts.colspan)
                                : 1
                    })
				...
				*/
                    ...
                    Object.entries(objTableGrid).forEach(([rIdx, rowObj]) => {
                        ...
                        if (
                            Array.isArray(objTabOpts.rowH) &&
                            objTabOpts.rowH[rIdx]
                        )
                            intRowH = inch2Emu(Number(objTabOpts.rowH[rIdx]))
                        else if (
                            objTabOpts.rowH &&
                            !isNaN(Number(objTabOpts.rowH))
                        )
                            ...
                            intRowH =
                                (slideItemObj.options.h
                                    ...
                                    : typeof slideItemObj.options.cy ===
                                      'number'
                                    ? slideItemObj.options.cy
                                    : 1) / arrTabRows.length
                        ...
                        Object.entries(rowObj).forEach(([_cIdx, cellObj]) => {
                            ...
                            let cellOpts =
                                cell.options || ({} as ITableCell['options'])
