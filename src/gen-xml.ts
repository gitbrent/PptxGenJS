...
export function makeXmlNotesMaster (): string {
	// NOTE: This is the minimal notesMaster structure PowerPoint produces after repairing a PptxGenJS-generated deck.
	// Previously this function emitted 6 placeholder shapes (Header / Date / Slide Image / Body / Footer / Slide Number)
	// that PowerPoint flagged as invalid and stripped during the repair dialog. Emitting only the elements PowerPoint
	// keeps (background, empty spTree, clrMap, notesStyle) avoids the repair prompt entirely. Per-slide notesSlide files
	// continue to declare their own placeholders (see `makeXmlNotesSlide`), so notes rendering is unaffected.
