...
function makeChartType(
	...
	data: Array<{ index: number, name: string, 	labels:Array<string>,
		values:Array<number>,
		sizes:Array<number>
 }>,
	opts: IChartOpts,
	...
	switch (chartType) {
		...
		case 'scatter':
			...
			*/
			...
			var colorIndex = -1;
			data.filter((_obj, idx) => { return idx > 0; }).forEach((obj, idx) => {
				colorIndex++;
				strXml += '<c:ser>';
				strXml += '  <c:idx val="' + idx + '"/>';
				strXml += '  <c:order val="' + idx + '"/>';
				...
				// 3: "Values": Scatter Chart has 2: `xVal` and `yVal`
				{
					...
					strXml += '      <c:ptCount val="' + data[0].values.length + '"/>';
					data[0].values.forEach((_value, idx) =>{ strXml += '<c:pt idx="' + idx + '"><c:v>' + (obj.values[idx] || obj.values[idx] == 0 ? obj.values[idx] : '') + '</c:v></c:pt>'; });
					strXml += '    </c:numCache>';
					strXml += '  </c:numRef>';
					strXml += '</c:yVal>';
				}
...
function makeCatAxis(opts, axisId, valAxisId) {
	...
	strXml += (opts.catGridLine !== 'none' ? createGridLineElement(opts.catGridLine, DEF_CHART_GRIDLINE, 'major') : '');
...
function makeValueAxis(opts, valAxisId) {
	...
	if (opts.valGridLine != 'none') strXml += createGridLineElement(opts.valGridLine, DEF_CHART_GRIDLINE, 'major');
	// '<c:title>' comes between '</c:majorGridlines>' and '<c:numFmt>'
...
function makeSerAxis(opts, axisId, valAxisId) {
	...
	strXml += '  <c:axPos val="' + (opts.barDir == 'col' ? 'b' : 'l') + '"/>';
	strXml += (opts.serGridLine !== 'none' ? createGridLineElement(opts.serGridLine, DEF_CHART_GRIDLINE, 'major') : '');
	// '<c:title>' comes between '</c:majorGridlines>' and '<c:numFmt>'
...
export function makeXmlPresentationRels() {
	var intRelNum = 0;
	var strXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + CRLF;
	strXml += '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">';
 ...
 * @return {string} strXml - slide OOXML
*/
export function makeXmlMaster(objSlide: ISlide): string {
	// NOTE: Pass layouts as static rels because they are not referenced any time
	var layoutDefs = gObjPptx.slideLayouts.map((_layoutDef, idx) => {
 ...
 * @return {String} complete XML string ready to be saved as a file
 */
export function makeXmlSlideLayoutRel(layoutNumber: number): string {
 ...
 * @return {String} complete XML string ready to be saved as a file
 */
export function makeXmlSlideRel(slideNumber: number): string {
	return gObjPptxGenerators.slideObjectRelationsToXml(
 ...
 * @return {String} complete XML string ready to be saved as a file
 */
export function makeXmlMasterRel(masterSlideObject: ISlide): string {
	var defaultRels = gObjPptx.slideLayouts.map((_layoutDef, idx) => {
 ...
 */
function getLayoutIdxForSlide(slideNumber: number): number {
	var layoutName = gObjPptx.slides[slideNumber - 1].layoutName;
...
function getShapeInfo(shapeName) {
	...
	var objShape = Object.keys(gObjPptxShapes).filter((key:string) => { return gObjPptxShapes[key].name == shapeName || gObjPptxShapes[key].displayName; })[0];
	if (typeof objShape !== 'undefined' && objShape != null) return objShape;

	return gObjPptxShapes.RECTANGLE;
}

export function createHyperlinkRels(inText, slideRels) {
	var arrTextObjects = [];
