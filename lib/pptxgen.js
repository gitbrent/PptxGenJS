...
/***/ function(module, exports, __webpack_require__) {
	...
	function getSmartParseNumber(inVal, inDir) {
	    ...
	    if (typeof inVal == 'string' && inVal.indexOf('%') > -1) {
	        if (inDir && inDir == 'X') return Math.round(parseInt(inVal, 10) / 100 * _slide2.default.gObjPptx.pptLayout.width);
	        if (inDir && inDir == 'Y') return Math.round(parseInt(inVal, 10) / 100 * _slide2.default.gObjPptx.pptLayout.height);
	        // Default: Assume width (x/cx)
	        return Math.round(parseInt(inVal, 10) / 100 * _slide2.default.gObjPptx.pptLayout.width);
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function makeXmlSlide(inSlide) {
	    ...
	    if (inSlide.slide.group) {
	        var startGroup,
	            endGroup,
	            group = new _slideGroup2.default().generateGroup();
	    ...
	    $.each(objSlideData, function (idx, slideObj) {
	        ...
	        // A: Set option vars
	        if (slideObj.options) {
	            var _OptionAdapter = (0, _optionAdapter2.default)(slideObj),
	                x = _OptionAdapter.x,
	                y = _OptionAdapter.y,
...
/***/ function(module, exports) {
	...
	exports.default = function (idx, slideObj, locationAttr, x, y, cx, cy) {
	    ...
	    var strSlideXml = ['<p:pic>', '<p:nvPicPr>', '<p:cNvPr id="' + (idx + 2) + '" name="Object ' + (idx + 1) + '" descr="' + slideObj.image + '"/>', '<p:cNvPicPr>', '<a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/>', '</p:nvPicPr>', '<p:blipFill>', '<a:blip r:embed="rId' + slideObj.imageRid + '" cstate="print"/><a:stretch><a:fillRect/></a:stretch>', '</p:blipFill>', '<p:spPr>', '<a:xfrm' + locationAttr + '>', '<a:off  x="' + x + '"  y="' + y + '"/>', '<a:ext cx="' + cx + '" cy="' + cy + '"/>', '</a:xfrm>', '<a:prstGeom prst="rect">', '<a:avLst/>', '</a:prstGeom>', '</p:spPr>', '</p:pic>'];
	
	    return strSlideXml.join('');
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function (slideObj) {
	    var EMU = 914400;
	    var x = 0,
	        y = 0,
	        cx = EMU * 10,
	    ...
	    if (shapeType && shapeType.name === 'polyline') {
	        ...
	        if (slideObj.options.x || slideObj.options.x == 0) x = (0, _helpers.getSmartParseNumber)(slideObj.options.x, 'X', _slide2.default.gObjPptx);
	        if (slideObj.options.y || slideObj.options.y == 0) y = (0, _helpers.getSmartParseNumber)(slideObj.options.y, 'Y', _slide2.default.gObjPptx);
	    }
	    if (slideObj.options.cx || slideObj.options.cx == 0) cx = (0, _helpers.getSmartParseNumber)(slideObj.options.cx, 'X', _slide2.default.gObjPptx);
	    if (slideObj.options.cy || slideObj.options.cy == 0) cy = (0, _helpers.getSmartParseNumber)(slideObj.options.cy, 'Y', _slide2.default.gObjPptx);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SlideGroup = function (_BaseGroup) {
	    ...
	    function SlideGroup() {
	        ...
	        _this.allShapes = _slide2.default.gObjPptx.slides;
	        return _this;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Shape = function () {
	    function Shape(shapeObject) {
	        ...
	        this.coordinate = (0, _optionAdapter2.default)(this.shapeObject);
	    }
	    ...
	    _createClass(Shape, [{
	        ...
	        value: function txBody(inSlide) {
	            ...
	            if (moreStyles != '') outStyles = '<a:pPr' + moreStylesAttr + '>' + moreStyles + '</a:pPr>';else if (moreStylesAttr != '') outStyles = '<a:pPr' + moreStylesAttr + '/>';
	
	            if (styleData != '') strSlideXml += '<p:style>' + styleData + '</p:style>';
	
	            if (typeof this.shapeObject.text == 'string') {
	
	                this._xmlShape += '<p:txBody>' + (0, _helpers.genXmlBodyProperties)(this.shapeObject.options) + '<a:lstStyle/><a:p>' + outStyles;
	                ...
	                for (var j = 0, total_size_j = this.shapeObject.text.length; j < total_size_j; j++) {
	                    if (_typeof(this.shapeObject.text[j]) == 'object' && this.shapeObject.text[j].text) {
	                        this._xmlShape += (0, _helpers.genXmlTextCommand)(this.shapeObject.text[j].options || this.shapeObject.options, this.shapeObject.text[j].text, inSlide.slide, outBodyOpt, outStyles, inSlide.slide.getPageNumber());
	                    } else if (typeof this.shapeObject.text[j] == 'string') {
	                        this._xmlShape += (0, _helpers.genXmlTextCommand)(this.shapeObject.options, this.shapeObject.text[j], inSlide.slide, outBodyOpt, outStyles, inSlide.slide.getPageNumber());
	        ...
	        key: 'genaratePathList',
	        value: function genaratePathList() {
	
	            var aPath = this.shapeObject.options && this.shapeObject.options.customGeom,
	                w = this.coordinate.cx,
	                h = this.coordinate.cy,
	                pathList = '<a:pathLst><a:path w="' + (0, _helpers.inch2Emu)(w) + '" h="' + (0, _helpers.inch2Emu)(h) + '">';
	            for (var nIndex = 0; nIndex < aPath.length; nIndex++) {
	            ...
	            function moveTo(points) {
	                var aPoints = points.substr(1).split(','),
	                    moveTo = ['<a:moveTo>', '<a:pt x="' + (0, _helpers.inch2Emu)(aPoints[0] / 96) + '" y="' + (0, _helpers.inch2Emu)(aPoints[1] / 96) + '"/>', '</a:moveTo>'];
	                return moveTo.join('');
	            }
	
	            function lnTo(points) {
	                var aPoints = points.substr(1).split(','),
	                    lineTo = ['<a:lnTo>', '<a:pt x="' + (0, _helpers.inch2Emu)(aPoints[0] / 96) + '" y="' + (0, _helpers.inch2Emu)(aPoints[1] / 96) + '"/>', '</a:lnTo>'];
	                return lineTo.join('');
	            }
	            ...
	            function quadBezTo(points) {
	                ...
	                for (var i = 0; i < aPoints.length; i++) {
	                    var apts = aPoints[i].split(',');
	                    quadBezTo += '<a:pt x="' + (0, _helpers.inch2Emu)(apts[0] / 96) + '" y="' + (0, _helpers.inch2Emu)(apts[1] / 96) + '"/>';
	                }
