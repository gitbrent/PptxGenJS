...
function demos() {
	...
	// Constants
	let CUST_NAME = "S.T.A.R. Laboratories";
	let USER_NAME = "Barry Allen";
	let COLOR_RED = "FF0000";
	let COLOR_AMB = "F2AF00";
	let COLOR_GRN = "7AB800";
	let COLOR_CRT = "AA0000";
	let COLOR_BLU = "0088CC";
	let COLOR_UNK = "A9A9A9";
	let ARRSTRBITES = [130];
	let CHARSPERLINE = 130; // "Open Sans", 13px, 900px-colW = ~19 words/line ~130 chars/line
	// Lorem text / base64 images
	//{
	let gStrLoremEnglish =
	...
	//}
	let gArrNamesF = ["Markiplier", "Jack", "Brian", "Paul", "Ev", "Ann", "Michelle", "Jenny", "Lara", "Kathryn"];
	let gArrNamesL = ["Johnson", "Septiceye", "Lapston", "Lewis", "Clark", "Griswold", "Hart", "Cube", "Malloy", "Capri"];
	let gStrHello = "BONJOUR - CIAO - GUTEN TAG - HELLO - HOLA - NAMASTE - OLÀ - ZDRAS-TVUY-TE - こんにちは - 你好";
	let gOptsTabOpts = { x: 0.5, y: 0.13, colW: [9, 3.33] }; // LAYOUT_WIDE w=13.33
	let gOptsTextL = { color: "9F9F9F", margin: 3, border: [null, null, { pt: "1", color: "CFCFCF" }, null] };
	let gOptsOptsR = { color: "9F9F9F", margin: 3, border: [0, 0, { pt: "1", color: "CFCFCF" }, 0], align: "right" };
	let gOptsTextR = { text: "PptxGenJS", options: gOptsOptsR };
	let gOptsCode = { color: "9F9F9F", margin: 3, border: { pt: "1", color: "CFCFCF" }, fill: "F1F1F1", fontFace: "Courier", fontSize: 12 };
	let gOptsSubTitle = { x: 0.5, y: 0.7, w: 4, h: 0.3, fontSize: 18, fontFace: "Arial", color: "0088CC", fill: "FFFFFF" };
	let gDemoTitleText = { fontSize: 14, color: "0088CC", bold: true };
	let gDemoTitleTextBk = { fontSize: 14, color: "0088CC", bold: true, breakLine: true };
	let gDemoTitleOpts = { fontSize: 13, color: "9F9F9F" };
	let gPaths = {
		...
		// STEP 4: Create Master Slides (from the old `pptxgen.masters.js` file - `gObjPptxMasters` items)
		{
			...
			pptx.defineSlideMaster({
				...
				objects: [
					...
					{
						text: {
							options: { x: 0, y: 6.9, w: "100%", h: 0.6, align: pptxgen.TEXT_HALIGN.center, valign: ("middle" as pptxgen.VAlign), color: "FFFFFF", fontSize: 12 },
							text: "S.T.A.R. Laboratories - Confidential"
						}
					},
					{
						placeholder: {
							options: { name: "title", type: (pptxgen.PLACEHOLDER_TYPES.title), x: 0.6, y: 0.2, w: 12, h: 1.0 },
							text: ""
						}
					},
					{
						placeholder: {
							options: { name: "body", type: (pptxgen.PLACEHOLDER_TYPES.body), x: 0.6, y: 1.5, w: 12, h: 5.25 },
	...
	function genSlides_Table(pptx) {
		// SLIDE 1: Table text alignment and cell styles
		{
			...
			// DEMO: align/valign -------------------------------------------------------------------------
			let objOpts1 = { x: 0.5, y: 0.7, w: 4, fontSize: 18, fontFace: "Arial", color: "0088CC" };
			...
			// DEMO: cell styles --------------------------------------------------------------------------
			let objOpts2 = { x: 6.0, y: 0.7, w: 4, fontSize: 18, fontFace: "Arial", color: "0088CC" };
			...
			let objOpts3 = { x: 0.5, y: 3.6, fontSize: 18, fontFace: "Arial", color: "0088CC" };
		...
		// SLIDE 3: Super rowspan/colspan demo
		{
			...
			let optsRowspan2 = { rowspan: 2, fill: "99FFCC" };
			let optsRowspan3 = { rowspan: 3, fill: "99FFCC" };
			let optsRowspan4 = { rowspan: 4, fill: "99FFCC" };
			let optsRowspan5 = { rowspan: 5, fill: "99FFCC" };
			let optsColspan2 = { colspan: 2, fill: "9999FF" };
			let optsColspan3 = { colspan: 3, fill: "9999FF" };
			let optsColspan4 = { colspan: 4, fill: "9999FF" };
			let optsColspan5 = { colspan: 5, fill: "9999FF" };
			...
			let arrTabRows5 = [
				...
				["C5", "D5", "E5", "F5", { text: "G5 -> H5", options: { colspan: 2, fill: "9999FF" } }, "J5", "K5", "L5"]
			];
		...
		// SLIDE 4: Cell Formatting / Cell Margins
		{
			...
			slide.addTable([[{ text: "no border and number zero", options: { margin: 5 } }, { text: 0, options: { margin: 5 } }]], {
				x: 0.5,
				y: 1.9,
				fill: "f2f9fc",
				border: { type: "none" },
			...
			let arrBorder = [{ color: "FF0000", pt: 1 }, { color: "00ff00", pt: 3 }, { color: "0000ff", pt: 5 }, { color: "9e9e9e", pt: 7 }];
			slide.addTable([["Borders!"]], {
				x: 0.5,
				y: 4.3,
				w: 12.3,
				rowH: 1.5,
		...
		// SLIDE 5: Cell Word-Level Formatting
		{
			...
			let opts2 = { x: 2.5, y: 5, h: 2, align: "center", valign: "middle", colW: [1.5, 1.5, 6], border: { pt: "1" }, fill: "F1F1F1" };
			let arrTabRows = [
				[
		...
		// ======== -----------------------------------------------------------------------------------
		{
			...
			let slide = pptx.addSlide();
			slide.addNotes("API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-tables.html");
			slide.addText([{ text: "Table Examples: ", options: gDemoTitleText }, { text: "Auto-Paging Example", options: gDemoTitleOpts }], {
				x: 0.5,
				y: 0.13,
				w: "90%"
			});
			...
			let slide = pptx.addSlide();
			slide.addText([{ text: "Table Examples: ", options: gDemoTitleText }, { text: "Smaller Table Area", options: gDemoTitleOpts }], {
				x: 0.5,
				y: 0.13,
				w: "90%"
			});
			...
			let slide = pptx.addSlide();
			slide.addText(
				[{ text: "Table Examples: ", options: gDemoTitleText }, { text: "Test: `{ newSlideStartY: 1.5 }`", options: gDemoTitleOpts }],
				{ x: 0.5, y: 0.13, w: "90%" }
			);
			slide.addTable(arrRows, {
				x: 3.0,
			...
			let slide = pptx.addSlide("MASTER_PLAIN", { bkgd: "CCFFCC" });
			slide.addText(
				[{ text: "Table Examples: ", options: gDemoTitleText }, { text: "Master Page with Auto-Paging", options: gDemoTitleOpts }],
				{ x: 0.5, y: 0.13, w: "90%" }
			);
			slide.addTable(arrRows, { x: 1.0, y: 0.6, colW: [0.75, 1.75, 7], margin: 5, border: { color: "CFCFCF" }, autoPage: true });

			let slide = pptx.addSlide();
			slide.addText([{ text: "Table Examples: ", options: gDemoTitleText }, { text: "Auto-Paging Disabled", options: gDemoTitleOpts }], {
				x: 0.5,
				y: 0.13,
				w: "90%"
			});
			...
			slide.addText(
				[
					{ text: "Table Examples: Char Weight Options", options: gDemoTitleTextBk },
					{ text: "autoPageLineWeight:0.25", options: gDemoTitleOpts }
				],
				{ x: 4.75, y: 0.13, w: 4, h: 0.4 }
			);
	...
	function genSlides_Image(pptx) {
		...
		// SLIDE 1: Image Types -----------------------------------------------------------------------------------
		{
			...
			slide.addImage({
				...
				w: 1.5,
				h: 1.5,
				path: NODEJS ? gPaths.gifAnimTrippy.path.replace(/http.+\/examples/, "../common") : gPaths.gifAnimTrippy.path
			});
			...
			slide.addImage({
				...
				w: 1.2,
				h: 1.2,
				path: NODEJS ? gPaths.ccDjGif.path.replace(/http.+\/examples/, "../common") : gPaths.ccDjGif.path
			});
		...
		// SLIDE 4: Image URLs -----------------------------------------------------------------------------------
		{
			...
			Object.keys(gOptsCode).forEach(function(key) {
			...
			// TOP-RIGHT:
			let objCodeEx2 = { x: 6.9, y: 0.6, w: 6.0, h: 0.6 };
			Object.keys(gOptsCode).forEach(function(key) {
			...
			// BTM-LEFT:
			let objCodeEx3 = { x: 0.5, y: 4.2, w: 12.4, h: 0.8 };
			Object.keys(gOptsCode).forEach(function(key) {
			...
			if (typeof window !== "undefined" && window.location.href.indexOf("gitbrent") > 0) {
				...
				slide.addImage({
					path: NODEJS ? gPaths.ccLicenseComp.path.replace(/http.+\/examples/, "../common") : gPaths.ccLicenseComp.path,
					x: 6.6,
					y: 3.2,
					w: 6.3,
					h: 3.7
	...
	function genSlides_Shape(pptx) {
		...
		slide.addText("OVAL (rotate:90, alpha:75)", {
			shape: pptx.shapes.OVAL,
			x: 7.7,
			y: 1.4,
			w: 3.0,
	...
	function genSlides_Text(pptx) {
		// SLIDE 1: Text alignment, percent x/y, etc.
		{
			...
			slide.addText([{ text: "(left/top)", options: { fontSize: 12 } }, { text: "Textbox", options: { bold: true } }], {
				x: 10,
				y: 0.75,
				w: 3.0,
				h: 1.0,
			...
			slide.addText([{ text: "Textbox" }, { text: "(right/bottom)", options: { fontSize: 12 } }], {
				x: 10,
				y: 2.25,
				w: 3.0,
				h: 1.0,
		...
		// SLIDE 2: Multi-Line Formatting, Line Breaks, Line Spacing
		{
			...
			slide.addText(
				[
					...
					{
						text: "Verdana 48",
						options: { fontSize: 48, fontFace: "Verdana", color: pptx.colors.ACCENT3, align: "left", breakLine: true }
					},
					{
						text: "Strikethrough",
						options: { fontSize: 36, fontFace: "Arial", color: pptx.colors.ACCENT4, align: "center", strike: true, breakLine: true }
					},
					{
						text: "Underline",
						options: { fontSize: 36, fontFace: "Arial", color: pptx.colors.ACCENT4, align: "center", underline: true, breakLine: true }
					},
		...
		// SLIDE 4: Hyperlinks, Text Shadow, Text Outline, Text Glow
		{
			...
			slide.addNotes("API Docs: https://gitbrent.github.io/PptxGenJS/docs/api-text.html");
			slide.addTable(
				[[{ text: "Text Examples: Hyperlinks, Text Shadow, Text Outline, Text Glow", options: gOptsTextL }, gOptsTextR]],
				gOptsTabOpts
			);
			...
			slide.addText([{ text: "Link to Slide #4", options: { hyperlink: { slide: "4" } } }], {
				x: 9.5,
				y: 6.4,
				w: 3,
				h: 0.6,
		...
		// SLIDE 5: Scheme Colors
		{
			...
			slide.addText("ACCENT3", { x: 0.5, y: 4.7, w: 6.0, h: 1.0, color: "FFFFFF", fill: pptx.colors.ACCENT3 });
			slide.addText("ACCENT4", { x: 7.0, y: 4.7, w: 6.0, h: 1.0, color: "FFFFFF", fill: pptx.colors.ACCENT4 });
	...
	function genSlides_Master(pptx) {
		let slide1 = pptx.addSlide("TITLE_SLIDE");
		...
		let slide6 = pptx.addSlide("THANKS_SLIDE");
	...
	if ( pptx.masters && Object.keys(pptx.masters).length > 0 ) {
		...
		let slide5 = pptx.addSlide( pptx.masters.MASTER_SLIDE, { bkgd:{ path:'https://raw.githubusercontent.com/gitbrent/PptxGenJS/v2.1.0/examples/images/title_bkgd_alt.jpg' } } );
