...
gulp.task("min-css", () => {
	...
	return gulp
		.src(["../css/hybrid.min.css", "./build/PptxGenJS/css/main.css"])
...
gulp.task("min-html", function () {
	...
	return gulp
		.src("build/PptxGenJS/index.html")
		...
		.pipe(replace(new RegExp(/<script>.*\(function\(i.*'pageview'\);\n+\s+<\/script>/, "s"), gTag2020, { logs: { enabled: true } }))
...
// Build/Deploy
gulp.task("default", gulp.series(gulp.parallel("deploy-css", "deploy-help", "deploy-html", "deploy-img", "deploy-index", "deploy-sitemap"), "min-css", "min-html"), () => {
	console.log("Done");
});
